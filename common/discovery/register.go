...
func (s *ServiceRegister) UpdateValue(val *EndpointInfo) error {
	...
	s.val = value
	logger.CtxInfof(*s.ctx, "lease success leaseID:%d, Put key:%s, val: %s, success!", s.leaseID, s.key, s.val)
	return nil
}
...
func (s *ServiceRegister) Close() error {
	...
	logger.CtxInfof(*s.ctx, "lease close! leaseID:%d, Put key:%s, val:%s", s.leaseID, s.key, s.val)
	return s.cli.Close()
}
